...
def all_recipe_posts(request):
    ...
    return render(request, 'recipes/all_recipe_posts.html', {'recipe_posts':recipe_posts})


def single_recipe_post(request, pk):
    recipe_post = Recipe_Post.objects.get(pk=pk)
    return render(request, 'recipes/single_recipe_post.html', {'recipe_post':recipe_post})
